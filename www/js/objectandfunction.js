    ...
    displayCompetitors: function() {
        ...
        for (var i=0; i<4; i++) {
            ...
            if (this.competitors[i]) {
                ...
                $('#' + selector_bestman_divId + ' .ui-block-b h3').text(this.competitors[i].name);  // Insert bestman name in Blk B
                ...
                $('#' + selector_bestman_divId + ' .ui-block-b h3').text('');
    ...
    storeAndDisplayCompetitor4ConfirmPage: function() {
        ...
        for (var j=0; j<4; j++) {
            if (this.competitors[j]) {
                $('div.cls-page61-best' + j + '-div .ui-block-b h3').text(this.competitors[j].name);
...
// Envelope Close Main
function Enevelope_close(envelopeNum, phone, cb) {
    var enevelope_selector = '.cls-invite' + envelopeNum + envelopeNum;
    if (phone === 'galaxy') {
        $(enevelope_selector).css('top', low_px.galaxy_low_px);
    } else if (phone === 'galaxytab') {
        $(enevelope_selector).css('top', low_px.galaxytab_low_px);
    // } else if (iPhone === 'iPhX') {
...
function subSelectRightEnvelopeToOpen(a, b, c, d, phone) {
    // if ( $('.cls-invite' + a + a).css('top') != low_px.galaxy_low_px) {
        Enevelope_close(a, phone, Enevelope_close_callback);
    // }
    // if ( $('.cls-invite' + b + b).css('top') != low_px.galaxy_low_px) {
        Enevelope_close(b, phone, Enevelope_close_callback);
    // }
    // if ( $('.cls-invite' + c + c).css('top') != low_px.galaxy_low_px ) {
        Enevelope_close(c, phone, Enevelope_close_callback);
    // }
    // if ( $('.cls-invite' + c + c).css('top') != low_px.galaxy_low_px ) {
    //     Enevelope_close(c, phone, Enevelope_close_callback);
    // }
    // if ( $('.cls-invite' + d + d).css('top') != low_px.galaxy_low_px ) {
        Enevelope_close(d, phone, Enevelope_close_callback);
    // }
}
// Select Right Enevelope to Open - SubFunction2
function subSelectorTogetherHandling(a, phone) {
    if (phone === 'galaxy') {
        $('.cls-invite' + a + a).css('top', high_px.galaxy_high_px);
    } else if (phone === 'galaxytab') {
        $('.cls-invite' + a + a).css('top', high_px.galaxytab_high_px);
    } else {}
...
function selectRightEnvelopeToOpen(dom_elmt, phone) {
    if ($(dom_elmt).attr('class') == 'cls-outer11') {
        subSelectorTogetherHandling(1, phone);
        subSelectRightEnvelopeToOpen(2, 3, 4, 5, phone);
    } else if ($(dom_elmt).attr('class') == 'cls-outer22') {
        subSelectorTogetherHandling(2, phone);
        subSelectRightEnvelopeToOpen(1, 3, 4, 5, phone);
    } else if ($(dom_elmt).attr('class') == 'cls-outer33') {
        subSelectorTogetherHandling(3, phone);
        subSelectRightEnvelopeToOpen(1, 2, 4, 5, phone);
    } else if ($(dom_elmt).attr('class') == 'cls-outer44') {
        subSelectorTogetherHandling(4, phone);
        subSelectRightEnvelopeToOpen(1, 2, 3, 5, phone);
    } else if ($(dom_elmt).attr('class') == 'cls-outer55') {
        subSelectorTogetherHandling(5, phone);
        subSelectRightEnvelopeToOpen(1, 2, 3, 4, phone);
    } else {}
}
// Close Enevelope
function selectEnvelopeToClose(dom_elmt, phone) {
    // var temp_string_dom = $(dom_elmt).attr('class');
    var temp_num_dom = parseInt($(dom_elmt).attr('class').slice(19, 20));
    Enevelope_close(temp_num_dom, phone, Enevelope_close_callback);
...
function transferDataLocalStorage4Submit() {
    ...
    // temp_faveu_data.country= window.localStorage.getItem("faveu_country");
    temp_faveu_data.country = window.localStorage.getItem("faveu_phonecountry") ? window.localStorage.getItem("faveu_phonecountry") : '';
...
var chatRoom = {
    ...
    displayChatRoomName: function(roomname) {
        $('#id-pagesucchat1-text1 .ui-block-b h3').text(roomname)
    },
